...
import_script = project_root / "scripts" / "import_styleflow.sh"
...
with col1:
    ...
    if not exists:
        st.warning(
            "未检测到 StyleFlow 项目。请先执行: "
            f"`bash {import_script.as_posix()}`"
        )

with col2:
    st.subheader("2) 快速导入命令")
    st.code(f"bash {import_script.as_posix()}", language="bash")
